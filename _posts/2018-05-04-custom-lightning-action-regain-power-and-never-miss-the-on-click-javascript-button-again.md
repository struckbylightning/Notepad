...
#### Needing to do following currently hurts as there\`s no straightforward way to create,

- A button with simple 'Are you sure?' confirm box.
- A button with conditional 'Are you sure?' confirm box.
- A button which does conditional redirection.
- A button that simply does API callouts to an external system (no UI required!)
- A button which opens up a prefilled form.
- A button which just calls an Apex method to do the needful.
- A button that can leverage the power of OOB [events that are handled in the Salesforce mobile app and Lightning Experience](https://developer.salesforce.com/docs/atlas.en-us.lightning.meta/lightning/events_one.htm) 
- A button that can redirect to another Lightning Component or open up the component inside a modal.
